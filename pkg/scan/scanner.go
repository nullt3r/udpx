					...
					func() {
						...
						for _, payload := range probe.Payloads {
							...
							if recv_length != 0 {
								...
								s.Result <- fmt.Sprintf(`{"address": "%s", "hostname": "%s", "protocol": "udp", "portid": "%d", "port_state": "open", "service_name": "%s", "service_product": null, "service_version": null, "extrainfo": "%s"}`, ip, domain, port, probe.Name, utils.DataToReadableFormat(recv_Data))
								return
							}
						}
					}()
				...
				func() {
					for _, payload := range probe.Payloads {
						...
						if recv_length != 0 {
							...
							s.Result <- fmt.Sprintf(`{"address": "%s", "hostname": null, "protocol": "udp", "portid": %d, "port_state": "open", "service_name": "%s", "service_product": null, "service_version": null, "extrainfo": "%s"}`, ip, port, probe.Name, utils.DataToReadableFormat(recv_Data))
							return
						}
					}
				}()
